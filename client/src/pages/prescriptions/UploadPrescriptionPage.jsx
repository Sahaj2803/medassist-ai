...
function UploadPrescriptionPage() {
  ...
  const handleUpload = async () => {
    ...
    setProgress(0);
    try {
      const { prescription } = await prescriptionService.upload(file, (evt) => {
        if (evt.total) setProgress(Math.round((evt.loaded / evt.total) * 100));
      });
      toast.success(t("prescriptions.uploadedToastSuccess"));
  ...
  return (
    <div className="container-shell max-w-2xl py-16">
      <span className="section-eyebrow">{t("prescriptions.uploadEyebrow")}</span>
      <h1 className="mt-3 text-3xl font-bold text-white">{t("prescriptions.scanNew")}</h1>
      <p className="mt-2 text-mist-300">{t("prescriptions.uploadDescription")}</p>
      ...
      >
        {uploading ? t("prescriptions.uploading").replace("{progress}", progress) : t("prescriptions.uploadAndScan")}
      </button>
    </div>
  );
}
